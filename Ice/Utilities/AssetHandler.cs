...
public static class AssetHandler
{
	...
	public static Material LoadMaterial(string bundleName, string materialName)
	{
		...
		if ((UnityEngine.Object)(object)val2 == (UnityEngine.Object)null)
		{
			UnityEngine.Debug.LogWarning((object)("[Ice] No usable material for '" + materialName + "' in bundle '" + bundleName + "'"));
			return null;
		}
	...
	public static IEnumerator LoadEmbeddedAudioClip(string resourceName, Action<AudioClip> callback)
	{
		...
		if ((int)audioType == 0)
		{
			UnityEngine.Debug.LogWarning((object)("[Ice] Unsupported audio format: " + ext));
			yield break;
		}
		...
		try
		{
			...
			if (stream == null)
			{
				UnityEngine.Debug.LogWarning((object)("[Ice] Could not find embedded resource: " + resourceName));
			}
			else
			{
		...
		{
			UnityEngine.Debug.LogWarning((object)("[Ice] Failed writing embedded audio temp file: " + ex));
		}
		if (!writeSuccess)
		{
		...
		try
		{
			...
			if ((int)www.result == 1)
			{
				...
				{
					UnityEngine.Debug.LogWarning((object)("[Ice] Clip loaded but data not ready: " + resourceName));
			...
			{
				UnityEngine.Debug.LogWarning((object)("[Ice] Failed to load audio clip '" + resourceName + "': " + www.error));
			}
		}
		finally
		{
