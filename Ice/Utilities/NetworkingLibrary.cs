...
public class NetworkingLibrary : MonoBehaviour
{
	...
	private const byte ICE_USER_EVENT = 69;

	private static readonly HashSet<string> detectedIceUsers = new HashSet<string>();
	...
	public static void BroadcastIceUser()
	{
		if (PhotonNetwork.InRoom)
		{
	...
	public static void HandleIceBroadcast(EventData photonEvent)
	{
		...
		if (!(text2 == PhotonNetwork.LocalPlayer.UserId))
		{
			...
			if (detectedIceUsers.Add(item))
			{
				NotificationLib.SendNotification(NotificationLib.NotificationType.Alert, "`" + text + "` Is An Ice User");
			}
		}
	}
	...
	public static void OnEvent(EventData photonEvent)
	{
		if (PhotonNetwork.InRoom)
		{
			...
			{
				HandleIceBroadcast(photonEvent);
			}
			else
			{
	...
	{
		return new List<string>(detectedIceUsers);
	}

	private void OpenLink(string url)
	{
		UnityEngine.Debug.LogWarning((object)("[Ice] Blocked external URL open: " + url));
	}
