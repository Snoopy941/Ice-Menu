...
internal class UpdateChecker : MonoBehaviour
{
	...
	private const string CacheLastCheckKey = "Ice_UpdateChecker_VersionFile_LastCheckUtc";

	private const string CacheLatestVersionKey = "Ice_UpdateChecker_VersionFile_LatestVersion";
	...
	private IEnumerator CheckForUpdates()
	{
		...
		req.timeout = 10;
		req.SetRequestHeader("User-Agent", "Ice-Mod-Menu/" + PluginInfo.menuVersion);
		yield return req.SendWebRequest();

		if ((int)req.result != 1)
		{
			UnityEngine.Debug.LogWarning((object)("[Ice] Update check failed: " + req.error));
			yield break;
		}
		...
		{
			UnityEngine.Debug.LogWarning((object)"[Ice] Update check returned an empty version file.");
			yield break;
		}
	...
	private static void HandleVersion(string latestVersion)
	{
		...
		{
			UnityEngine.Debug.Log((object)("[Ice] Update check: current version is up to date (" + PluginInfo.menuVersion + ")."));
			return;
		}

		string message = "Update available: " + latestVersion + " (current " + PluginInfo.menuVersion + ")";
		UnityEngine.Debug.Log((object)("[Ice] " + message + " - https://github.com/Wyvern-Softworks/NXO-Mod-Menu"));
