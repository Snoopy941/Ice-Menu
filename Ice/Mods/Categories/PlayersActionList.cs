...
public class PlayersActionList
{
	...
	public static void AddPlayerCam()
	{
		...
		if ((UnityEngine.Object)(object)PlayerCam == (UnityEngine.Object)null)
		{
			GameObject val2 = new GameObject("Ice Photon.Realtime.Player Camera");
		...
		((UnityEngine.Object)PlayerCamScreen).name = "Ice Photon.Realtime.Player Camera Screen";
	...
	public static void SpectatePlayer()
	{
		...
		else if ((UnityEngine.Object)(object)Visuals.iceCamera == (UnityEngine.Object)null)
		{
			Visuals.iceCamera = new GameObject("Ice_Cam");
			Visuals.iceCamera.transform.position = ((Component)Variables.taggerInstance.headCollider).transform.position;
			Camera val = Visuals.iceCamera.GetComponent<Camera>() ?? Visuals.iceCamera.AddComponent<Camera>();
			val.nearClipPlane = 0.01f;
			val.cameraType = (CameraType)1;
			Visuals.iceCamera.transform.position = vRRigFromNetPlayer.bodyTransform.TransformPoint(new Vector3(0f, 0.8f, -1.5f));
			Visuals.iceCamera.transform.rotation = vRRigFromNetPlayer.headMesh.transform.rotation;
		}
		else
		{
			Camera val = Visuals.iceCamera.GetComponent<Camera>() ?? Visuals.iceCamera.AddComponent<Camera>();
