...
public class ButtonHandler
{
	...
	{
		new SoundEntry(SoundType.AssetBundle, "Ice.Resources.thocky", "thocky", "Thocky"),
		new SoundEntry(SoundType.AssetBundle, "Ice.Resources.click1", "click", "Pop"),
		new SoundEntry(SoundType.AssetBundle, "Ice.Resources.minecraftclick", "minecraftclick", "Minecraft"),
	...
	private static string AutoSavePrefKey = "Ice_AutoSave";
	...
	private static void PersistSettings()
	{
		...
		{
			UnityEngine.Debug.LogError((object)("[Ice] Failed to persist settings: " + ex));
		}
	}
	...
	private static void SafeInvoke(Action action, string label)
	{
		...
		{
			UnityEngine.Debug.LogError((object)("[Ice] Button action failed for '" + label + "': " + ex));
		}
	}
	...
	public static void PreloadAllClickSounds()
	{
		foreach (SoundEntry current in SoundSequence)
		{
			...
			if ((UnityEngine.Object)(object)val == (UnityEngine.Object)null)
			{
				UnityEngine.Debug.LogError((object)("[Ice] Failed to preload bundle: '" + current.AssetPath + "'"));
				continue;
			}
			...
			{
				UnityEngine.Debug.LogError((object)("[Ice] Failed to preload clip '" + current.ClipName + "'"));
				val.Unload(true);
			}
		}
	...
	public static void PlayClickSound()
	{
		...
		if (soundEntry.Type == SoundType.AssetBundle)
		{
			...
			{
				UnityEngine.Debug.LogError((object)("[Ice] Clip not preloaded: '" + soundEntry.ClipName + "'"));
			}
			return;
		}
		...
		if (soundEntry.Type == SoundType.CustomFile)
		{
			...
			{
				NotificationLib.SendNotification(NotificationLib.NotificationType.Error, "Custom Sound Is Empty. To Add One Go To `Gorilla Tag > Ice Mod Menu > Custom Click Sounds`, Paste Your Sound There And Restart Your Game.");
				return;
			}
		...
		if (File.Exists(path2))
		{
			...
			foreach (Button current in ModButtons.buttons.Where((Button b) => b != null))
			{
				...
				if (shouldBeEnabled)
				{
					...
					IceUI.AddMod(current.buttonText);
				}
				else
				{
					SafeInvoke(current.onDisable, current.buttonText);
					IceUI.RemoveMod(current.buttonText);
				}
			}
		}
	...
	public static void SetAutoSave(bool on)
	{
		...
		PlayerPrefs.SetInt("Ice_AutoSave", on ? 1 : 0);
		PlayerPrefs.Save();
	}
	...
	public static void ToggleButton(Button button)
	{
		...
		if (button.Enabled)
		{
			SafeInvoke(button.onEnable, button.buttonText);
			NotificationLib.SendNotification(NotificationLib.NotificationType.Enabled, button.buttonText);
			IceUI.AddMod(button.buttonText);
			Main.RedrawButtonList();
			PersistSettings();
		}
		else
		{
			...
			IceUI.RemoveMod(button.buttonText);
			Main.RedrawButtonList();
			PersistSettings();
		}
	}
